...
function userReplyHtml(opts: { name: string; subject: string; logoCid: string }) {
  const { name, subject, logoCid } = opts;
    ...
    <tr>
      <td align="center" style="padding:24px;">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;background:#ffffff;border-radius:10px;box-shadow:0 2px 10px rgba(0,0,0,.05);">
          ...
          <tr>
            <td style="padding:0 24px 8px 24px;" align="left">
          ...
          <tr>
            <td style="padding:0 24px 16px 24px;" align="left">
              ...
              <p style="margin:0 0 18px 0;font-family:Arial,Helvetica,sans-serif;font-size:13px;line-height:1.6;color:#6b7280;">
                If this wasn’t you, please reply to this email and let us know.
              </p>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <td style="padding:0 24px 8px 24px;" align="left">
              ...
              <p style="margin:0 0 8px 0;font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:1.6;color:#111827;">
                <a href="https://www.teleringer.com" style="color:#0b7d5c;text-decoration:none;" target="_blank">www.teleringer.com</a>
              </p>
            </td>
          </tr>
          <tr>
...
function userReplyText(name: string, subject: string) {
  const safeName = name || "there";
  return (
    `Hi ${safeName},\n\n` +
    `Thanks for contacting Teleringer. We received your message` +
    ...
    `Teleringer\nTel: (570) 456-5550\nwww.teleringer.com\n\n` +
    `---\n` +
    `The information contained in this electronic mail transmission is a privileged attorney-client communication and is intended solely for the addressee(s) named above... (full text in HTML version).`
  );
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const transporter = nodemailer.createTransport({
      ...
      port: Number(reqEnv("SMTP_PORT")),     // 465 or 587
      secure: String(reqEnv("SMTP_SECURE")) === "true",
      auth: {
        user: reqEnv("SMTP_USER"),           // full mailbox
        pass: reqEnv("SMTP_PASS"),
      },
      // Optional hardening for some cPanel servers:
    ...
    // 2) Auto-reply to the visitor (HTML + text, with inline logo via CID)
    if (email) {
      ...
      await transporter.sendMail({
        ...
        text: userReplyText(name, subject),
        html: userReplyHtml({ name, subject, logoCid }),
        attachments: [
          {
            // Use an https URL to your header logo (or switch to a local file path on the server)
            filename: "logo.png",
            path: reqEnv("MAIL_LOGO_URL"), // e.g. https://www.teleringer.com/assets/logo-header.png
            cid: logoCid,
          },
        ],
      });
