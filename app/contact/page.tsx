...
export default function ContactPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (formData.service.length) fd.set('service', formData.service.join(', '));
  ...
  return (
    <main style={{ maxWidth: 840, margin: '40px auto', padding: '0 16px', lineHeight: 1.6 }}>
      ...
      <form onSubmit={handleSubmit} style={{ marginTop: 24, display: 'grid', gap: 12 }}>
        ...
        <input name="company" placeholder="Company (optional)" value={formData.company} onChange={handleChange} />
        ...
        <fieldset style={{ border: '1px solid #ddd', padding: 12 }}>
          ...
          <label style={{ display: 'block', marginBottom: 6 }}>
            <input type="checkbox"
              ...
              onChange={() => handleServiceToggle('VoIP / Phone System')} /> VoIP / Phone System
          </label>
          <label style={{ display: 'block', marginBottom: 6 }}>
            <input type="checkbox"
              checked={formData.service.includes('Website')}
              onChange={() => handleServiceToggle('Website')} /> Website
          </label>
          <label style={{ display: 'block' }}>
            <input type="checkbox"
              checked={formData.service.includes('AI Concierge / Call Agent')}
              onChange={() => handleServiceToggle('AI Concierge / Call Agent')} /> AI Concierge / Call Agent
          </label>
        </fieldset>
        ...
        {submitStatus === 'ok' && <p role="status" style={{ color: 'green' }}>Thank you! Your message was sent.</p>}
        {submitStatus === 'error' && <p role="status" style={{ color: 'crimson' }}>Something went wrong. Please try again.</p>}
      </form>
    </main>
  );
}
